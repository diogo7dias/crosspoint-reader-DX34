...
namespace {
// Production wiring for the pure settings codec (persist/SettingsCodec.cpp): the
// three impure dependencies the codec needs, captured behind the SettingsEnv
// port. Lives here (the Hal-coupled TU) so the codec itself stays host-buildable.
const crosspoint::persist::SettingsEnv& productionSettingsEnv() {
  static const crosspoint::persist::SettingsEnv env{
      [] { return (uint8_t)getLanguageCount(); },
      [](const std::string& p) { return std::string(obfuscation::obfuscateToBase64(p).c_str()); },
      [](const std::string& b, bool* ok) { return obfuscation::deobfuscateFromBase64(b.c_str(), ok); },
      [](bool on) { setBitmapHelpersUseFactoryLUT(on); },
  };
  return env;
}
}  // namespace
...
void JsonSettingsIO::populateSettingsDoc(const CrossPointSettings& s, JsonDocument& doc) {
  crosspoint::persist::encodeSettings(s, doc, productionSettingsEnv());
}
...
bool JsonSettingsIO::loadSettings(CrossPointSettings& s, const char* json, bool* needsResave) {
  return crosspoint::persist::decodeSettings(s, json, productionSettingsEnv(), needsResave);
}
