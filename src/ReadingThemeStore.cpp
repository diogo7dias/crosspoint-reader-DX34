...
uint8_t clampRange(const uint8_t value, const uint8_t minValue, const uint8_t maxValue, const uint8_t defaultValue) {
...
bool ReadingThemeStore::loadFromFile() {
  ...
  if (!JsonSettingsIO::loadReadingThemes(*this, json.c_str())) {
    ...
    LOG_INF("RTH", "loadFromFile: recovered %d themes from .bak", (int)themes.size());
  }
  loadedFromFile = true;
  return true;
}

ReadingTheme ReadingThemeStore::fromSettings(const std::string& name, const CrossPointSettings& settings) {
  ReadingTheme theme;
  ...
  theme.statusBarBookPercentagePosition = settings.statusBarBookPercentagePosition;
  theme.statusBarChapterPercentagePosition = settings.statusBarChapterPercentagePosition;
...
ReadingTheme ReadingThemeStore::captureCurrent(const std::string& name) const { return fromSettings(name, SETTINGS); }
...
void ReadingThemeStore::applyThemeToSettings(const ReadingTheme& theme, CrossPointSettings& settings) {
  ...
  settings.screenMarginHorizontal = clampRange(theme.screenMarginHorizontal, 0, 55, 20);
  settings.screenMarginTop = clampRange(theme.screenMarginTop, 0, 55, 20);
  settings.screenMarginBottom = clampRange(theme.screenMarginBottom, 0, 55, 20);
  settings.paragraphAlignment = clampRange(
      theme.paragraphAlignment, 0, CrossPointSettings::PARAGRAPH_ALIGNMENT_COUNT - 1, CrossPointSettings::JUSTIFIED);
  settings.extraParagraphSpacingLevel =
      clampRange(theme.extraParagraphSpacingLevel, 0, CrossPointSettings::EXTRA_PARAGRAPH_SPACING_COUNT - 1,
                 CrossPointSettings::EXTRA_SPACING_M);
  settings.wordSpacingPercent = clampRange(theme.wordSpacingPercent, 0, CrossPointSettings::WORD_SPACING_MODE_COUNT - 1,
                                           CrossPointSettings::WORD_SPACING_NORMAL);
  settings.firstLineIndentMode =
      clampRange(theme.firstLineIndentMode, 0, CrossPointSettings::FIRST_LINE_INDENT_MODE_COUNT - 1,
  ...
  settings.statusBarPageCounterMode =
      CrossPointSettings::normalizeStatusBarPageCounterMode(theme.statusBarPageCounterMode);
  settings.statusBarShowBookPercentage = theme.statusBarShowBookPercentage ? 1 : 0;
  settings.statusBarShowChapterPercentage = theme.statusBarShowChapterPercentage ? 1 : 0;
  ...
  settings.statusBarNoTitleTruncation = theme.statusBarNoTitleTruncation ? 1 : 0;
  settings.statusBarBatteryPosition =
      clampRange(theme.statusBarBatteryPosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_LEFT);
  settings.statusBarProgressTextPosition =
      clampRange(theme.statusBarProgressTextPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  settings.statusBarPageCounterPosition =
      clampRange(theme.statusBarPageCounterPosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  settings.statusBarBookPercentagePosition =
      clampRange(theme.statusBarBookPercentagePosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  settings.statusBarChapterPercentagePosition =
      clampRange(theme.statusBarChapterPercentagePosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  settings.statusBarBookBarPosition =
      clampRange(theme.statusBarBookBarPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  settings.statusBarChapterBarPosition =
      clampRange(theme.statusBarChapterBarPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  settings.statusBarTitlePosition =
      clampRange(theme.statusBarTitlePosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  settings.statusBarTextAlignment =
      clampRange(theme.statusBarTextAlignment, 0, CrossPointSettings::STATUS_TEXT_ALIGNMENT_COUNT - 1,
...
bool ReadingThemeStore::matchesCurrent(const ReadingTheme& theme) const {
  ...
  return current.fontFamily == theme.fontFamily && current.fontSize == theme.fontSize &&
         ...
         current.screenMarginTop == theme.screenMarginTop && current.screenMarginBottom == theme.screenMarginBottom &&
         ...
         current.statusBarShowBookPercentage == theme.statusBarShowBookPercentage &&
         current.statusBarShowChapterPercentage == theme.statusBarShowChapterPercentage &&
         current.statusBarShowBookBar == theme.statusBarShowBookBar &&
         current.statusBarShowChapterBar == theme.statusBarShowChapterBar &&
         current.statusBarShowChapterTitle == theme.statusBarShowChapterTitle &&
         current.statusBarNoTitleTruncation == theme.statusBarNoTitleTruncation &&
         current.statusBarBatteryPosition == theme.statusBarBatteryPosition &&
         current.statusBarProgressTextPosition == theme.statusBarProgressTextPosition &&
         current.statusBarPageCounterPosition == theme.statusBarPageCounterPosition &&
         current.statusBarBookPercentagePosition == theme.statusBarBookPercentagePosition &&
         current.statusBarChapterPercentagePosition == theme.statusBarChapterPercentagePosition &&
         current.statusBarBookBarPosition == theme.statusBarBookBarPosition &&
         current.statusBarChapterBarPosition == theme.statusBarChapterBarPosition &&
...
std::string ReadingThemeStore::makeUniqueName(const std::string& desiredName, const int ignoreIndex) const {
...
bool ReadingThemeStore::renameTheme(const size_t index, const std::string& desiredName) {
  if (index >= themes.size()) {
    return false;
  }
...
void ReadingThemeStore::sortByName() {
  std::sort(themes.begin(), themes.end(), [](const ReadingTheme& a, const ReadingTheme& b) {
    // Case-insensitive comparison
    const size_t len = std::min(a.name.size(), b.name.size());
    for (size_t i = 0; i < len; i++) {
      const char ca = static_cast<char>(std::tolower(static_cast<unsigned char>(a.name[i])));
      const char cb = static_cast<char>(std::tolower(static_cast<unsigned char>(b.name[i])));
...
bool ReadingThemeStore::applyTheme(const size_t index, const std::string& cachePath) {
  if (index >= themes.size()) {
    return false;
  }
...
bool ReadingThemeStore::canRevertTheme(const std::string& cachePath) const {
  return hasRevertTheme && (revertThemeCachePath.empty() || revertThemeCachePath == cachePath);
}
...
bool ReadingThemeStore::saveCurrentBookSettings(const std::string& cachePath) {
  ...
  return JsonSettingsIO::saveReadingTheme(fromSettings(instance.lastAppliedThemeName, SETTINGS),
                                          bookReaderSettingsPath(cachePath).c_str());
}

bool ReadingThemeStore::loadBookSettings(const std::string& cachePath, ReadingTheme& theme) {
  if (cachePath.empty()) {
    return false;
  }
...
ReadingTheme ReadingThemeStore::normalizeTheme(const ReadingTheme& theme) {
  ...
  normalized.fontFamily = CrossPointSettings::normalizeFontFamily(theme.fontFamily);
  ...
  normalized.paragraphAlignment = clampRange(
      theme.paragraphAlignment, 0, CrossPointSettings::PARAGRAPH_ALIGNMENT_COUNT - 1, CrossPointSettings::JUSTIFIED);
  normalized.extraParagraphSpacingLevel =
      clampRange(theme.extraParagraphSpacingLevel, 0, CrossPointSettings::EXTRA_PARAGRAPH_SPACING_COUNT - 1,
                 CrossPointSettings::EXTRA_SPACING_M);
  normalized.wordSpacingPercent =
      clampRange(theme.wordSpacingPercent, 0, CrossPointSettings::WORD_SPACING_MODE_COUNT - 1,
                 CrossPointSettings::WORD_SPACING_NORMAL);
  normalized.firstLineIndentMode =
      clampRange(theme.firstLineIndentMode, 0, CrossPointSettings::FIRST_LINE_INDENT_MODE_COUNT - 1,
                 CrossPointSettings::INDENT_BOOK);
  ...
  normalized.statusBarPageCounterMode =
      CrossPointSettings::normalizeStatusBarPageCounterMode(theme.statusBarPageCounterMode);
  normalized.statusBarShowBookPercentage = theme.statusBarShowBookPercentage ? 1 : 0;
  normalized.statusBarShowChapterPercentage = theme.statusBarShowChapterPercentage ? 1 : 0;
  ...
  normalized.statusBarShowChapterTitle = theme.statusBarShowChapterTitle ? 1 : 0;
  normalized.statusBarNoTitleTruncation = theme.statusBarNoTitleTruncation ? 1 : 0;
  normalized.statusBarBatteryPosition =
      clampRange(theme.statusBarBatteryPosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_LEFT);
  normalized.statusBarProgressTextPosition =
      clampRange(theme.statusBarProgressTextPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  normalized.statusBarPageCounterPosition =
      clampRange(theme.statusBarPageCounterPosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  normalized.statusBarBookPercentagePosition =
      clampRange(theme.statusBarBookPercentagePosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  normalized.statusBarChapterPercentagePosition =
      clampRange(theme.statusBarChapterPercentagePosition, 0, CrossPointSettings::STATUS_BAR_TEXT_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_BOTTOM_CENTER);
  normalized.statusBarBookBarPosition =
      clampRange(theme.statusBarBookBarPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  normalized.statusBarChapterBarPosition =
      clampRange(theme.statusBarChapterBarPosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  normalized.statusBarTitlePosition =
      clampRange(theme.statusBarTitlePosition, 0, CrossPointSettings::STATUS_BAR_ITEM_POSITION_COUNT - 1,
                 CrossPointSettings::STATUS_AT_BOTTOM);
  normalized.statusBarTextAlignment =
      clampRange(theme.statusBarTextAlignment, 0, CrossPointSettings::STATUS_TEXT_ALIGNMENT_COUNT - 1,
                 CrossPointSettings::STATUS_TEXT_RIGHT);
