...
struct SettingInfo {
  ...
  // Free-string labels appended after enumValues for ENUM types whose option
  // list grows at runtime. Display indices [0, enumValues.size()) resolve via
  // I18N; [enumValues.size(), enumValues.size()+dynamicLabels.size()) resolve
  // directly from this vector. Currently unused (no runtime-dynamic pickers
  // remain) but kept as the generic mechanism.
