...
// sizes 11/13/15/18 (their glyph tables stream from the SD pack — see
// SdFontManager Tier 2). ChareInk is excluded: it is the always-in-flash
// fallback floor and ships only the five flash sizes. In the default (non-SD)
// build, or for ChareInk, the extra sizes fold to the nearest kept size so a
// persisted value never lands on a font that does not exist.
