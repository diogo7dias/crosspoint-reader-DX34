...
# ---------------------------------------------------------------------------
# Web JSON dict generator
# ---------------------------------------------------------------------------

def load_web_key_list(translations_dir: str) -> List[str]:
    """Read _web_keys.txt; one StrId per line, blanks/comments ignored."""
    path = Path(translations_dir) / "_web_keys.txt"
    if not path.is_file():
        return []
    keys: List[str] = []
    seen = set()
...
def generate_web_dict_header(
    ...
    """Generate WebI18nDict.generated.h: per-language gzipped JSON dict + lookup."""
    # Validate web keys exist in translations.
    string_keys_set = set(string_keys)
    missing = [k for k in web_keys if k not in string_keys_set]
    if missing:
        raise ValueError(
            "Web keys missing from English translations: " + ", ".join(missing)
        )
    ...
    # Per-language gzipped JSON blob.
    blob_meta: List[Tuple[str, str, int]] = []  # (lang_code, var_name, size)
    for lang_idx, (code, name) in enumerate(zip(languages, language_names)):
        abbrev = get_lang_abbreviation(code, name)
        # Build dict for this language using ordered web_keys.
        # Use OrderedDict-like construction (json preserves insertion order).
        lang_dict = {k: translations[k][lang_idx] for k in web_keys}
        # Compact JSON: no spaces, ensure_ascii=False keeps utf-8 bytes
        # (gzip eats it efficiently and the browser parses it natively).
        payload = json.dumps(lang_dict, ensure_ascii=False, separators=(",", ":"))
        compressed = gzip.compress(payload.encode("utf-8"), compresslevel=9)

        var_name = f"WebI18nDict_{abbrev}"
        lines.append(f"// {name} ({len(payload)} B → {len(compressed)} B gz)")
        lines.append(f"constexpr unsigned char {var_name}[] PROGMEM = {{")
        for i in range(0, len(compressed), 16):
            chunk = compressed[i : i + 16]
            hex_values = ", ".join(f"0x{b:02x}" for b in chunk)
            lines.append(f"  {hex_values},")
        lines.append("};")
        lines.append(f"constexpr size_t {var_name}Size = {len(compressed)};")
        lines.append("")
        blob_meta.append((code, var_name, len(compressed)))

    # Lookup table: index by Language enum.
    lines.append("struct WebI18nBlob {")
    lines.append("  const unsigned char* data;")
    lines.append("  size_t size;")
    lines.append("};")
    lines.append("")
    lines.append("inline WebI18nBlob getWebI18nBlob(Language lang) {")
    lines.append("  switch (lang) {")
    for code, var_name, size in blob_meta:
        lines.append(f"    case Language::{code}:")
        lines.append(f"      return {{{var_name}, {var_name}Size}};")
    first_var = blob_meta[0][1]
    lines.append("    default:")
    lines.append(f"      return {{{first_var}, {first_var}Size}};")
    lines.append("  }")
    lines.append("}")
...
def main(translations_dir=None, output_dir=None) -> None:
    ...
    try:
        ...
        # Web JSON dict header — only emitted when _web_keys.txt is present.
        web_keys = load_web_key_list(translations_dir)
        if web_keys:
            generate_web_dict_header(
                languages, language_names, string_keys, translations, web_keys,
                str(out / "WebI18nDict.generated.h"),
            )
        ...
        if web_keys:
            print(f"  Web dict keys: {len(web_keys)}")
