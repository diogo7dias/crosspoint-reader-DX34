...
# Vollkorn, ALL sizes 10..18, all four real weights
...
#
# Usage:  bake-sd-vollkorn-packs.sh <output-dir>
#   e.g.  bash lib/EpdFont/scripts/bake-sd-vollkorn-packs.sh /tmp/sdpacks
set -euo pipefail
cd "$(dirname "$0")"
EPDFONT_DIR="$(cd .. && pwd)"

OUT="${1:?usage: bake-sd-vollkorn-packs.sh <output-dir>}"
mkdir -p "$OUT"
SCRATCH="$(mktemp -d)"
...
FAMILIES=(
  "vollkorn|regular:0,bold:1,italic:2,bolditalic:3"
)
