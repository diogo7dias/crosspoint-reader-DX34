#!/bin/bash
# Bake Tier-1 CPBN font packs (bitmap-only) for the EXTRA reader sizes
# (11/13/15/18) of every built-in reader family. These sizes ship as Tier-1 (same
# as 10/12/14/16/17): their glyph TABLES are compiled into flash (committed
# headers, offload_bitmaps.py drops the bitmap under slim), and only the DEFLATE
# bitmap blob lives in the .bin pack on the SD card — so they add ~0 heap at read
# time (tables are XIP from flash, not heap-loaded).
#
# Source of truth = the committed headers (lib/EpdFont/builtinFonts/*_{11,13,15,
# 18}_*.h), compiled WITHOUT CROSSPOINT_SD_FONTS_SLIM so their bitmap arrays are
# present to serialize from. No re-baking from TTF here, so the packs can never
# drift from the flash glyph tables. Each pack is round-tripped through the real
# Tier-1 loader (EpdBinFontLoader, with the count cross-check) before it is
# written, so a pack that would not load on device is never published.
#
...
FAMILIES=(
  "bookerly|regular:0,bold:1,italic:2"
  "georgia|regular:0,bold:1,italic:2"
  "lato|regular:0,bold:1,italic:2,bolditalic:3"
  "helvetica|regular:0,bold:1,italic:2"
  "verdana|regular:0,bold:1,italic:2"
)
