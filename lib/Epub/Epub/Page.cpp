...
std::unique_ptr<PageHorizontalRule> PageHorizontalRule::deserialize(FsFile& file) {
  ...
  if (width == 0 || thickness == 0) {
    LOG_ERR("PGE", "Deserialization failed: invalid horizontal rule (width=%u thickness=%u)", width, thickness);
    return nullptr;
  }
...
std::unique_ptr<Page> Page::deserialize(FsFile& file) {
  ...
  for (uint16_t i = 0; i < count; i++) {
    ...
    if (tag == TAG_PageLine) {
      ...
      if (!rule) {
        LOG_ERR("PGE", "Deserialization failed: PageHorizontalRule at index %u returned null", i);
        return nullptr;
      }
