...
struct JpegContext {
  ...
  // X and Y are tracked separately: a single (X-derived) scale applied to both
  // axes skips the wrong srcRow during nearest-neighbor downscaling when the
  // dest aspect ratio differs from source, losing image content.
  int32_t fineScaleFPX;  // X: src -> dst column mapping
  int32_t invScaleFPX;   // X: dst -> src column mapping
  int32_t fineScaleFPY;  // Y: src -> dst row mapping
  int32_t invScaleFPY;   // Y: dst -> src row mapping
...
bool JpegToFramebufferConverter::decodeToFramebuffer(const std::string& imagePath, GfxRenderer& renderer,
  ...
  // Progressive JPEGs decode DC-only (JPEGDEC forces JPEG_SCALE_EIGHTH on them):
  // the first scan carries the DC coefficients, yielding a 1/8-resolution image
  // that the fine-scale bilinear path upscales to fit. The two MCU_SKIP defects
  // in JPEGDEC's JPEGDecodeMCU_P (wild-pointer store-fault + Y-DC clobber) are
  // patched in scripts/patch_jpegdec.py, so this is crash-free and unclobbered.
  bool isProgressive = jpeg->getJPEGType() == JPEG_MODE_PROGRESSIVE;
  if (isProgressive) {
    LOG_INF("JPG", "Progressive JPEG - decoding DC coefficients only (lower quality): %s", imagePath.c_str());
  }
