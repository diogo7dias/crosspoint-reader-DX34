...
// These numeric enums mirror CrossPointSettings ORIENTATION / tiltPageTurn so
// the HAL stays independent of the app settings header. Values MUST match.
...
// QMI8658 gyro HAL (X3 only). Detects sharp left/right "flick" gestures for
// page turns. On the X4 begin() leaves _available=false and update() is a
// single-branch no-op, so the main loop pays nothing.
class HalTiltSensor {
