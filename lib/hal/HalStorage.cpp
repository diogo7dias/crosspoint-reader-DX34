...
HalStorage::HalStorage() {
  ...
  // openFileForRead/Write take the lock and then assign to a HalFile& out-param;
  // if that out-param already held an Impl, its destructor takes the lock again
  // to close the prior FsFile under serialization (see HalFile::Impl::~Impl
  // below). Priority inheritance still applies to recursive mutexes.
...
class HalFile::Impl {
 public:
  ...
  // SdFat is not thread-safe; FsFile::close() touches SD/SPI and must run under
  // StorageLock or it races SdSpiCard::m_spiActive across tasks and trips
  // FreeRTOS's xTaskPriorityDisinherit assert. The FsFile member destructor
  // (DESTRUCTOR_CLOSES_FILE=1) will close() again after the lock releases, but
  // close() on an already-closed FsFile is a no-op. See SdFat issue #518.
  ~Impl() {
    HalStorage::StorageLock lock;
    file.close();
  }
