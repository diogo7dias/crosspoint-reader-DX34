...
- [ ] **Footprint fix so heavy books actually OPEN (follow-up to the escape hotfix above).** The escape fix un-bricks the device but `__melomaniac__`'s book still can't be opened. **Correction from the architecture deep-dive (2026-06-03):** the earlier "CSS parser ~100 KB at render" was wrong — commit `3feac314` already disk-paged CSS (render holds only a ~few-KB `hashedIndex_` + 24-entry LRU; the 100 KB `rulesBySelector_` map is build-only). His `reader-render-oom` crashed in the glyph/render pass, *after* CSS-cache build, so the real remaining peak is the **per-paragraph layout working set** (`ParsedText` word/style vectors, 50-150 KB on a dense paragraph) + glyph prewarm. Architecture work spun out into two RFCs:
    - **RFC #164** — bounded layout-arena + degradation seam (caps the per-paragraph peak to a compile-time constant; reuses the dead 24 KB `layoutHeapAnchor_` for zero net heap; degrade trim-prewarm → greedy-break → no-hyphen → skip-images instead of OOM→restart). This is the real fix that makes the book open. Depends on RFC #163.
    - **RFC #163** — `MemoryPolicy` deep module: consolidate the 9 scattered heap thresholds + the OOM recovery ladder behind one host-testable surface. Foundation that provides the heap-gate → DegradeLevel mapping #164 consumes, and finally makes the Crash1/Crash2 ladder reproducible on host.
  - Still want `__melomaniac__`'s EPUB (fex.net link in Telegram) reproduced on the host reader-sim (SimHeap injector, phase-2 EPUB pipeline) to confirm the exact killer before landing #164.
