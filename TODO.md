...
- [ ] **Footprint fix so big-CSS / heavy books actually OPEN (follow-up to the escape hotfix above).** The escape fix un-bricks the device but `__melomaniac__`'s book still can't be opened — at OOM the book wanted ~120 KB of the 142 KB heap (CSS parser ~100 KB + layout + glyph prewarm). Needs his EPUB (he shared a fex.net link in Telegram) reproduced on the host reader-sim (SimHeap injector, phase-2 EPUB pipeline) to confirm the killer (likely the ~100 KB CSS parser) before cutting/streaming it or degrading render under low memory. Render is currently all-or-nothing (no skip-images / smaller-font-cache fallback).
