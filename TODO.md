...
**IN-FLIGHT (branch `feat/sd-font-packs`, host-tested, NOT flashed) — SD font packs: offload built-in reader fonts flash→SD to free ~1 MB OTA flash.** Fonts are ~2.15 MB (~40% of firmware); already 2-bit + DEFLATE so the win is structural, not better compression. Revives the CPBN format/loader removed in `80a239ce`. Staged Tier 1 (bitmap-only on SD, tables stay in flash) → Tier 2 (full pack). Deploy is **firmware-driven** (a System "Export font packs to SD" action writes `/fonts/*.bin` from the in-flash bitmaps — no card reader), enabling a safe 2-phase flash: Phase 1 fat build exports + reads SD with flash fallback (zero-risk validate), Phase 2 slim build drops the flash bitmaps for the win. Flag-gated `CROSSPOINT_SD_FONTS`, fallback floor = ChareInk + UI always in flash.
  - **DONE (committed, host-tested, zero device impact):** step 1 CPBN v2 format + `validateBlob` + `cpbn.py` serializer (`687ace6b`); step 2 `EpdBinFontLoader` core — DI `BlobSource`, streamed CRC, bounds-checked trampoline (`2286ea8f`); step 3a `EpdBinExport.h` in-flash→CPBN serializer for the bootstrap (`4c14eaf4`). 18 C++ + 5 py tests, `test_host` 265/265. New `test_bin_font_format` env **needs CI wiring**.
  - **TODO (firmware-heavy, daily-driver path — implement in small device-validated steps):** `src/fonts/` registry + single-active manager + `HalFileBlobSource` + `HalFile` export sink + flag-gated registration in `main.cpp` (mutable `EpdFontData` copy per active font) + System-menu export action + `fontconvert.py --bitmap-out` & conditional-bitmap emit + scoped Bookerly-pilot bake → Phase-1 flash + validate (render parity, Snappy-LAW, heap) → extend to 5 families → Phase-2 slim flash. Memory: `sd-font-packs-cpbn-inflight`.
