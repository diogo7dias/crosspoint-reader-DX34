...
**IN-FLIGHT (on `main`, code-complete + host-tested, NOT device-validated) — SD font packs: offload built-in reader fonts flash→SD to free OTA flash.** Fonts are ~2.15 MB (~40% of firmware); already 2-bit + DEFLATE so the win is structural, not better compression. Revives the CPBN format/loader removed in `80a239ce`. `CROSSPOINT_SD_FONTS` enables `SdFontManager` (read SD packs, flash fallback); `CROSSPOINT_SD_FONTS_SLIM` additionally drops the flash bitmaps. Fallback floor = ChareInk + UI always in flash. **`default` build is byte-unchanged (80.7%) — all flag-gated.**
  ...
  - **Tier 2 — PAYLOAD (NOT STARTED) — user wants every integer size 10–18 for all 5 families** (current set 10/12/14/16/17 → add 11/13/15/18 = 20 new (family,size) fonts, ~60 weight packs since most families are 3-weight). Plan, Georgia-first then scale (daily-driver guardrail):
    - **C2 (code wiring, gated to `CROSSPOINT_SD_FONTS`):** NO committed flash headers (true Tier-2, zero flash). New `*_FONT_ID`s via a NEW name-hash path in `build-font-ids.sh` (hash the canonical name triple, reproducible without a header; rely on `SECTION_FILE_VERSION` bumps if a font's metrics ever change since the id is content-independent). `EpdFont` stub objects start on the nearest chareink fallback; `insertFont` (ungated) + `registerFont(..., tablesInFile=true)` (gated). `fontSize` is a GLOBAL enum shared by all families → expand the size menu to 9 options (`fontSizeOptionCount`, display-index maps, `nextFontSize`, `fontSizeToPointSize`); make `normalizeFontSizeForFamily` FAMILY-AWARE (pass 11/13/15/18 through for baked families, fold to nearest for not-yet-baked ones); add the Georgia `getReaderFontId` cases. Section cache keys purely on fontId → auto-invalidates.
    - **C3 (bake + publish):** generate transient new-size headers via `fontconvert.py <name> <size> <ttf> --2bit --compress` (SKIP `dedup-shared-tables.py` so they stay self-contained; never run the family convert script wholesale — it would regenerate the existing 10/12/14/16/17 and risk byte drift). A Tier-2 pack tool (variant of `tools/pack_export.cpp` using `exportFontBlobWithTables`) emits the `.bin`s; publish to the `fonts` orphan branch + extend `manifest.json`. Georgia = 12 packs.
    - **C4 (device):** flash `sdfonts_slim`, download Georgia packs via the button, pick a new size, read + page-turn + bold/italic + fallback-when-missing; then batch the remaining 4 families. ⚠ asset deploy to `fonts` branch — warn before pushing.
