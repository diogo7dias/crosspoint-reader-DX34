...
- **`d10dceee` Text Anti-Aliasing toggle (re-added, default OFF).** Global opt-in toggle in Settings → Reader (on-device SettingsActivity + web), CrossPoint-style. Revived the neutered `textAntiAliasing` field: added to kFields, removed the force-to-0 in the JSON decoder AND in ReadingThemeStore theme-apply (both silently reverted it before), kept it OUT of the in-book theme menu (global-only). With AA off the render path is byte-identical to before → no page-turn regression. *Device-validate: toggle on → text edges smoother + sticks across reboot/book-open; off → page turns not slowed. NOTE: same subtle-on-this-panel effect that was cut before; kept as opt-in per user request.*
...
- **Swap `community-sdk` → `freeink-sdk` for literal 1:1 with CrossPoint (user-requested, later):** verified 2026-07-02 that our X3 driver is *already* byte-for-byte 1:1 with `Free-Ink/freeink-sdk` at the panel level — all 30 X3 LUTs identical (checked programmatically), same init/CDI/VCOM/power/refresh-dispatch lineage (freeink's `Uc8253X3Driver` header: "ported from the community-sdk main CrossPoint ships"; its credits = our 21 fork commits' authors). So the swap changes the *library wrapper only*, not a single SPI byte the X3 receives → **zero panel-level benefit.** Cost is high: freeink's API is a full rewrite (`FreeInkDisplay`/`PanelDriver`/`EpdBus` vs our `EInkDisplay`/`GfxRenderer`), and the **X4 daily driver rides the same stack** → real brick risk, multi-week gated refactor. User still wants it on principle; treat as a large architectural refactor under the safety protocol (build-flag gate, debug env first), NOT routine work. Clone at `Free-Ink/freeink-sdk`.
- **X3 refresh-policy fix — DONE, committed, device-validated on physical X3 (2026-07-02):** root-caused the X3 reading "bloom"/gray-snow (charge accumulation, not a defect, not the SDK). UC8253 partial refreshes (fast/half) are not DC-balanced; the periodic FULL clear X4 gets was missing on X3. Fix = CrossPoint's policy: fast page turns + one FULL (complete waveform) every N pages (`getRefreshFrequency`, default 15) for X3; X4 unchanged (still HALF every N). Applied in `EpubReaderActivity.cpp` + both spots in `XtcReaderActivity.cpp`; X4 path byte-identical. User confirmed ghosting fixed on device. Builds clean (Flash 81.4%). NOT released yet.
